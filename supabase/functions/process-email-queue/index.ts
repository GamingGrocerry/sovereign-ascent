...
function getRetryAfterSeconds(headers: Headers): number {
...
Deno.serve(async (req) => {
  ...
  if (!resendApiKey || !supabaseUrl || !supabaseServiceKey) {
    console.error('Missing required environment variables (RESEND_API_KEY, SUPABASE_URL, or SUPABASE_SERVICE_ROLE_KEY)')
    return new Response(
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const errorMsg = `Resend API error [${status}]: ${errorBody}`
          ...
          if (isRateLimited(status)) {
            ...
            const retryAfterSecs = getRetryAfterSeconds(response.headers)
            await supabase
              .from('email_send_state')
              .update({
          ...
          if (isForbidden(status)) {
            await moveToDlq(supabase, queue, msg, 'Resend API returned 403 Forbidden')
            return new Response(
          ...
          // Other errors — log failure, leave in queue for retry
