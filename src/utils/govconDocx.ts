...
// ═══════════════════════════════════════════════════════════════
export async function generateFarComplianceMatrixDocx(): Promise<Blob> {
  const d = new BrandedDocx({
    docCode: "EFC-DLV-001", title: "FAR Compliance Matrix Framework",
  ...
  d.instructionBox("HOW TO USE: For each applicable FAR clause — (1) confirm applicability, (2) select compliance status, (3) verify evidence is on file, (4) document the responsible owner and evidence reference.");

  d.sectionHeading("2  Contract Information");
  d.formFields([
    { label: "Contractor / Company Name" }, { label: "Contract Number" },
    { label: "Contracting Agency / Office" }, { label: "Period of Performance" },
    { label: "Contract Type (FFP / CPFF / T&M / IDIQ)" }, { label: "Compliance Review Date" },
  ]);

  d.sectionHeading("3  Compliance Status Key");
  d.statusKey();

  d.sectionHeading("4  FAR Compliance Matrix");
  const headers = ["#", "FAR Clause / Title", "Core Requirement", "Status", "Evidence", "Notes / Owner"];
  const widths = [6, 20, 28, 14, 10, 22];
  ...
  d.sectionHeading("5  Summary");
  d.formFields([{ label: "Total Compliant" }, { label: "Total Non-Compliant / Gaps" }]);
  d.signOff(["Reviewed By", "Contracts Lead", "Senior Management"]);
  d.disclaimer();
  return d.getBlob();
}
...
// ═══════════════════════════════════════════════════════════════
export async function generateProposalChecklistDocx(): Promise<Blob> {
  const d = new BrandedDocx({
    docCode: "EFC-DLV-002", title: "Proposal Compliance Checklist Framework",
  ...
  d.sectionHeading("1  Purpose & Scope");
  d.bodyText("This Proposal Compliance Checklist ensures all federal solicitation response packages — RFP, RFQ, or RFTOP — are complete, compliant, and submission-ready. Apply this checklist to each solicitation response to prevent administrative disqualification and strengthen proposal discipline.");
  d.instructionBox("HOW TO USE: Check each item as completed. Mark N/A in Notes if not applicable. All MANDATORY items must be addressed before submission. Assign a team owner to each section.");

  d.sectionHeading("2  Solicitation Details");
  d.formFields([
    { label: "Solicitation Number" }, { label: "Issuing Agency / Office" },
    { label: "Submission Deadline" }, { label: "Proposal Manager" },
    { label: "Contract Vehicle / NAICS Code" }, { label: "Set-Aside Type" },
  ]);

  d.sectionHeading("3  Pre-Submission Compliance Checklist");
  const h = ["✓", "Checklist Item", "FAR Reference", "Required?", "Notes / Owner"];
  const w = [5, 38, 16, 12, 29];

  d.subHeading("Section A — Administrative & Registration");
  d.table(h, [
  ...
  d.subHeading("Section B — Technical & Management Volume");
  d.table(h, [
  ...
  d.subHeading("Section C — Price / Cost Volume");
  d.table(h, [
  ...
  d.subHeading("Section D — Representations & Certifications");
  d.table(h, [
  ...
  d.subHeading("Section E — Administrative & Submission");
  d.table(h, [
  ...
  d.signOff(["Proposal Manager", "Contracts / Legal", "Authorizing Official"]);
  d.disclaimer();
  return d.getBlob();
}
...
// ═══════════════════════════════════════════════════════════════
export async function generateCmmcGapAssessmentDocx(): Promise<Blob> {
  const d = new BrandedDocx({
    docCode: "EFC-DLV-006", title: "CMMC 2.0 Gap Assessment Checklist Framework",
  ...
  d.sectionHeading("1  Purpose & Scope");
  d.bodyText("This CMMC 2.0 Gap Assessment Checklist supports organizations in evaluating their current cybersecurity posture against the 110 practices required under CMMC Level 2, aligned with NIST SP 800-171 Rev 2. It supports readiness for third-party assessment (C3PAO) and applies to all DoD contractors handling Controlled Unclassified Information (CUI).");
  d.bodyText("Following the regulatory overhaul effective February 1, 2026, contractors now fulfill assessment obligations exclusively through DFARS 252.204-7021. This framework reflects the current enforcement posture.");
  d.instructionBox("HOW TO USE: For each practice — (1) assess current status, (2) describe any gap, (3) assign a remediation owner and target date. This is a LIVING document — update continuously until assessment-ready.");

  d.sectionHeading("2  Organization & Assessment Details");
  d.formFields([
    { label: "Organization Name" }, { label: "CMMC Level Target" },
    { label: "Assessment Lead" }, { label: "Assessment Date" },
    { label: "CUI Systems / Enclaves Scope" }, { label: "Target C3PAO Assessment Date" },
    { label: "SPRS Score Estimate" }, { label: "C3PAO Firm (if selected)" },
  ]);

  d.sectionHeading("3  Compliance Status Key");
  d.statusKey();

  d.sectionHeading("4  CMMC Level 2 Practice Assessment");
  const h = ["ID", "Practice Name", "Core Requirement", "Status", "Gap Description", "Owner / Target"];
  const w = [10, 16, 26, 14, 18, 16];

  const families = [
    { name: "ACCESS CONTROL (AC)", practices: [
    ...
    { name: "AWARENESS & TRAINING (AT)", practices: [
    ...
    { name: "AUDIT & ACCOUNTABILITY (AU)", practices: [
    ...
    { name: "CONFIGURATION MANAGEMENT (CM)", practices: [
    ...
    { name: "IDENTIFICATION & AUTHENTICATION (IA)", practices: [
    ...
    { name: "INCIDENT RESPONSE (IR)", practices: [
    ...
    { name: "MEDIA PROTECTION (MP)", practices: [
    ...
    { name: "RISK ASSESSMENT (RA)", practices: [
    ...
    { name: "SYSTEM & COMMUNICATIONS (SC)", practices: [
    ...
    { name: "SYSTEM & INFORMATION INTEGRITY (SI)", practices: [
  ...
  families.forEach((fam) => { d.subHeading(fam.name); d.table(h, fam.practices, w); });

  d.sectionHeading("5  Assessment Summary");
  d.formFields([
    { label: "Total Compliant Practices" }, { label: "Total Partial" },
    { label: "Total Non-Compliant" }, { label: "Estimated SPRS Score" },
  ]);
  d.signOff(["CISO / IT Security Lead", "Compliance / Contracts", "Senior Management"]);
  d.disclaimer();
  return d.getBlob();
}
...
// ═══════════════════════════════════════════════════════════════
export async function generateCtipAuditChecklistDocx(): Promise<Blob> {
  const d = new BrandedDocx({
    docCode: "EFC-DLV-009", title: "CTIP 2026 Audit Readiness Checklist Framework",
  ...
  d.sectionHeading("1  Purpose & Regulatory Background");
  d.bodyText("The Combating Trafficking in Persons (CTIP) program is a federal requirement under FAR 52.222-50 and Executive Order 13627. All federal contractors and subcontractors are prohibited from engaging in trafficking-related activities. Contracts exceeding $550,000 with overseas performance require a formal Compliance Plan.");
  d.bodyText("This checklist prepares organizations for CTIP compliance audits and is aligned with the 2026 enforcement guidance. All items must be addressed and documented prior to audit.");
  d.instructionBox("HOW TO USE: Complete each section before contract performance begins and update annually. Check each box when addressed, assign an evidence reference, and note the responsible owner.");

  d.sectionHeading("2  Contract & Organization Details");
  d.formFields([
    { label: "Organization / Contractor Name" }, { label: "Contract Number(s)" },
    { label: "Contracting Agency" }, { label: "Performance Location(s)" },
    { label: "CTIP Compliance Officer" }, { label: "Compliance Plan Version & Date" },
  ]);

  d.sectionHeading("3  CTIP Audit Readiness Checklist");
  const h = ["✓", "Checklist Item", "Requirement", "Status", "Evidence Ref", "Owner"];
  const w = [5, 32, 16, 14, 16, 17];

  d.subHeading("Section A — Policy & Governance");
  d.table(h, [
  ...
  d.subHeading("Section B — Employee Awareness & Training");
  d.table(h, [
  ...
  d.subHeading("Section C — Recruitment & Employment Practices");
  d.table(h, [
  ...
  d.subHeading("Section D — Monitoring & Reporting");
  d.table(h, [
  ...
  d.subHeading("Section E — Subcontractor Oversight");
  d.table(h, [
  ...
  d.subHeading("Section F — Documentation & Record Retention");
  d.table(h, [
  ...
  d.sectionHeading("4  Audit Readiness Summary");
  d.formFields([
    { label: "Items Complete" }, { label: "Items In Progress" },
    { label: "Items Not Started" }, { label: "Audit Ready? (YES / NO)" },
  ]);
  d.signOff(["CTIP Compliance Officer", "Contracts Lead", "Senior Management"]);
  d.disclaimer();
  return d.getBlob();
}
